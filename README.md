# [Datanar](https://datanar.ru) - сервис по сокращению ссылок
...
Это не новая идея и уже существует множество сервисов с похожим функционалом,
вот их небольшой срез: 

| Сервис                                      | Регистрация | Кастомизация ссылки | QR-код  | Статистика | Русский язык | Пароль  | API     |
|---------------------------------------------|-------------|---------------------|---------|------------|--------------|---------|---------|
...
- Сократить ссылку
- Получить сразу сгенерированный QR-код
...
- `BlockedDomain` - регулярное выражение, описывающее URL-адреса, которые
...
- [Celery](Dockerfile) - контейнер, созданный на базе `Django`, но с 
  ...
  ссылок и динамического отслеживания срока годности перенаправлений
