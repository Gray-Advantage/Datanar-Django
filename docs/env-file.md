...
### DATANAR_SECRET_KEY
- Это большая случайная последовательность, применяемая для защиты от CSRF
...
- Через сколько дней после регистрации фоновая задача удаляет пользователя,
  так и не подтвердившего почту. По умолчанию равно сроку жизни ссылки из
  письма, то есть аккаунт удаляется, как только ссылка протухла
- Не трогает администраторов, пользователей с подтверждённой почтой, тех, кто
  хотя бы раз входил, и тех, у кого есть созданные ссылки
...
- Список сетей через запятую, из которых приложение доверяет заголовку
  с адресом клиента. Запросы приходят от nginx, поэтому без этого Django
  видит адрес прокси, а не посетителя, и всё, что считает по адресу -
  в первую очередь лимиты allauth на регистрацию и вход - складывает
  всех клиентов в один счётчик
- Пустое значение выключает подмену: адресом клиента будет считаться тот,
  с которого пришёл запрос
### DATANAR_REAL_IP_HEADER
- Имя заголовка с адресом клиента в формате WSGI. По умолчанию
  `HTTP_X_REAL_IP`, то есть заголовок `X-Real-IP`, который выставляет nginx.
  Для `X-Forwarded-For` укажите `HTTP_X_FORWARDED_FOR` - берётся первый
  адрес из списка
...
- Использовать ли в качестве базы данных SQLite3 (база данных в файле). Имеет
...
- Отладочный режим, при котором в консоль выводится дополнительная информация 
...
- Режим заглушки, при котором происходит полный цикл создания и обновления 
  сертификатов, за одним исключением: реальный запрос в центр сертификации не 
